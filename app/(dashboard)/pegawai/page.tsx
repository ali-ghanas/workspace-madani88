...
export default async function PegawaiListPage() {
  ...
  return (
    <div>
      ...
      {error && <p className="text-sm text-destructive">Gagal memuat pegawai: {error.message}</p>}
              ...
              return (
                <tr key={p.id} className="border-t border-border hover:bg-accent/40">
                  <td className="px-4 py-2">
                    <Link href={`/pegawai/${p.id}`} className="font-medium text-foreground hover:underline">
                      {p.nama}
                    </Link>
                    {!p.status_aktif && (
